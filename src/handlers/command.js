...
const commandhandler = (dir, handler, commandTable) => {
    if (!fs.existsSync(dir)) throw new Error(`zoro utils >> couldn't find commands directory`);
        ...
        for (const commandFile of commands) {
            if (commandTable) {
                ...
                if (check(File) === '✅') {
                    ...
                    console.log(colors.green(`Command handler >> Loaded "${File.name}"`));
                } else {
                    const { name } = path.parse(commandFile);
                    console.log(colors.red(`Command handler >> Failed to load "${name}"`));
                }
